...
function getSettings() {
  if (!extension_settings[SETTINGS_KEY]) {
    extension_settings[SETTINGS_KEY] = {
      ...
      hideTableEdit: true, panelWidth: 440, injectEnabled: true,
...
function isRowEmpty(row, colCount) {
  for (let i = 0; i < colCount; i++) { if ((row[i] || "").trim()) return false; }
  return true;
}
...
async function aiGenerate(userInstruction, mode) {
  ...
  try { return await generateRaw(userInstruction, "", false, false, systemPrompt); }
  catch (e) {
...
function renderLog(table, schemaEntry) {
  ...
  let iconClass = "default", iconLetter = "?";
  if (tableName.includes("임무") || tableName.includes("mission")) { iconClass = "mission"; iconLetter = "M"; }
  else if (tableName.includes("이벤트") || tableName.includes("event") || tableName.includes("의식")) { iconClass = "event"; iconLetter = "E"; }
  else if (tableName.includes("전투") || tableName.includes("combat")) { iconClass = "combat"; iconLetter = "⚔"; }
...
function renderOverview() {
  const tables = getTables(); const schema = getSchema();
  const modeLabels = { timeline: "⏱ 타임라인", profile: "👤 프로필", log: "📋 로그", grid: "⊞ 그리드", table: "▦ 테이블", custom: "✎ 커스텀" };
  let h = "";
  for (let i = 0; i < schema.length; i++) {
    const t = tables[i]; if (!t) continue;
...
function renderRaw() {
  ...
  for (const [idx, table] of Object.entries(tables)) h += `<div class="fab-card"><div class="fab-ch">테이블 ${idx}: ${table.name}</div>${renderPlainTable(table)}</div>`;
  return h;
}
...
function renderSettings() {
  ...
  for (const [key, label] of [["accent","액센트"],["insert","Insert"],["update","Update"],["delete","Delete"]]) {
...
async function handleAiAction(mode) {
  ...
  try {
    ...
    if (!response) { if (statusEl) statusEl.innerHTML = '<span class="fab-gen-err">AI 응답 실패.</span>'; return; }
    const ops = parseEdits(response);
    if (!ops.length) {
      if (statusEl) statusEl.innerHTML = '<span class="fab-gen-err">유효한 명령 없음.</span>';
      if (previewEl) previewEl.innerHTML = `<div class="fab-gen-raw"><div class="fab-gen-raw-label">원본:</div><pre>${esc(response.substring(0, 2000))}</pre></div>`;
      return;
    }
    ...
    for (const op of ops) {
      ...
      else if (op.type === "update") ph += `<div class="fab-gen-op update">~ update → T${op.ti}[${op.ri}]</div>`;
      else if (op.type === "delete") ph += `<div class="fab-gen-op delete">- delete → T${op.ti}[${op.ri}]</div>`;
    }
...
function setupDelegation() {
  ...
  content.addEventListener("click", (e) => {
    ...
    switch (act) {
      ...
      case "del-table": {
        ...
        if (schema.length <= 1) { alert("최소 1개."); break; }
      ...
      case "add-col": {
        ...
        if (schema[ti]) { schema[ti].columns.push(`Col${schema[ti].columns.length + 1}`); setSchema(schema); refreshPanel(); }
        break;
      }
      case "del-col": {
        const ti = parseInt(action.dataset.ti), ci = parseInt(action.dataset.ci); const schema = getSchema();
        if (schema[ti]?.columns.length > 1) { schema[ti].columns.splice(ci, 1); setSchema(schema); refreshPanel(); }
        break;
      }
      case "add-table": {
      ...
      case "reset-schema": { if (confirm("기본값 복원?")) { resetSchema(); refreshPanel(); updateExtSlot(); } break; }
      ...
      case "json-apply": {
        ...
        if (!raw) { statusEl.innerHTML = '<span class="fab-gen-err">JSON 입력 필요.</span>'; break; }
        try {
          ...
          statusEl.innerHTML = `<span class="fab-gen-ok">✅ ${ns.length}개 교체 완료.</span>`; refreshPanel();
        } catch (err) { statusEl.innerHTML = `<span class="fab-gen-err">${err.message}</span>`; }
        break;
      }
      ...
      case "ai-cancel": {
        ...
        if (p) p.innerHTML = ""; if (s) s.innerHTML = '<span class="fab-gen-info">취소됨.</span>';
        break;
      }
    }
  });
  ...
  content.addEventListener("change", (e) => {
    ...
    else if (t.id === "fab-opt-width") { getSettings().panelWidth = parseInt(t.value) || 440; saveSettings(); applyPanelWidth(); }
    else if (t.id === "fab-opt-inject") { getSettings().injectEnabled = t.checked; saveSettings(); const ts = document.getElementById("fab-inject-tables"); if (ts) ts.classList.toggle("fab-disabled", !t.checked); updateExtSlot(); }
    else if (t.dataset.injectIdx !== undefined) { getSettings().injectTables[parseInt(t.dataset.injectIdx)] = t.checked; saveSettings(); updateExtSlot(); }
    else if (t.id === "fab-opt-depth") { getSettings().injectDepth = parseInt(t.value) || 4; saveSettings(); }
  });
...
function createExtSlot() {
  const container = document.getElementById("extensions_settings2"); if (!container) return;
  ...
  wrapper.querySelector(".inline-drawer-toggle").addEventListener("click", function () {
    const c = wrapper.querySelector(".inline-drawer-content"); const a = wrapper.querySelector(".inline-drawer-icon.down");
    const o = c.style.display !== "none"; c.style.display = o ? "none" : "block";
  ...
  const hideChk = document.getElementById("fab-ext-chk-hide"); hideChk.checked = getSettings().hideTableEdit;
  hideChk.addEventListener("change", () => { getSettings().hideTableEdit = hideChk.checked; saveSettings(); });
  const injectChk = document.getElementById("fab-ext-chk-inject"); injectChk.checked = getSettings().injectEnabled;
...
function updateExtSlot() {
  ...
  const h = document.getElementById("fab-ext-chk-hide"); if (h) h.checked = settings.hideTableEdit;
  const i = document.getElementById("fab-ext-chk-inject"); if (i) i.checked = settings.injectEnabled;
}
...
function applyPanelWidth() {
  ...
  panel.style.width = w + "px"; panel.style.right = panelOpen ? "0" : `-${w + 20}px`;
}
...
function togglePanel() { panelOpen = !panelOpen; const w = getSettings().panelWidth || 440; const p = document.getElementById("fab-panel"); if (p) p.style.right = panelOpen ? "0" : `-${w + 20}px`; if (panelOpen) refreshPanel(); }
...
jQuery(async () => {
  ...
  eventSource.on(event_types.MESSAGE_RECEIVED, (idx) => { const ctx = getContext(); const msg = ctx.chat[idx]; if (msg?.mes) processMsg(msg.mes); setTimeout(hideBlocks, 300); });
